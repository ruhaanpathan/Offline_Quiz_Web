...
<div class="page-wrapper animate-fade">
    <div class="page-header">
        <div>
            <a href="/teacher/quiz_report.php?id=<?= $quizId ?>"
                style="color:var(--text-secondary);font-size:0.85rem;">← Back to Report</a>
            <h1><?= sanitize($quiz['title']) ?> — Full Detail</h1>
            <p style="color:var(--text-muted);"><?= sanitize($quiz['class_name']) ?> • <?= sanitize($quiz['subject']) ?>
                • Code: <?= sanitize($quiz['quiz_code']) ?> • <?= $totalStudents ?> students</p>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <a href="/teacher/ajax/export_csv.php?quiz_id=<?= $quizId ?>&type=results"
                class="btn btn-outline btn-sm">Results CSV</a>
            <a href="/teacher/ajax/export_csv.php?quiz_id=<?= $quizId ?>&type=attendance"
                class="btn btn-outline btn-sm">Attendance CSV</a>
            <a href="/teacher/ajax/export_csv.php?quiz_id=<?= $quizId ?>&type=answers"
                class="btn btn-outline btn-sm">Answers CSV</a>
    ...
    <div class="card" style="margin-bottom:24px;">
        <div class="card-header">
            ...
            <div style="display:flex;gap:12px;">
                <span style="display:flex;align-items:center;gap:6px;font-size:0.85rem;"><span
                        style="width:10px;height:10px;border-radius:50%;background:var(--accent-green);display:inline-block;"></span>
                    Present: <strong><?= $presentCount ?></strong></span>
                <span style="display:flex;align-items:center;gap:6px;font-size:0.85rem;"><span
                        style="width:10px;height:10px;border-radius:50%;background:#ff3860;display:inline-block;"></span>
                    Absent: <strong><?= $absentCount ?></strong></span>
                <span style="font-size:0.85rem;color:var(--text-muted);">Total:
                    <strong><?= count($enrolledStudents) ?></strong></span>
            </div>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px 0;">
            <?php foreach ($enrolledStudents as $es):
                ...
                ?>
                <div
                    style="display:flex;align-items:center;gap:8px;padding:8px 14px;border-radius:8px;background:<?= $isPresent ? 'rgba(20, 205, 60, 0.1)' : 'rgba(255,56,96,0.08)' ?>;border:1px solid <?= $isPresent ? 'rgba(72,199,142,0.2)' : 'rgba(255,56,96,0.15)' ?>;font-size:0.85rem;">
                    <span style="font-size:0.9rem;"><?= $isPresent ? '' : '' ?></span>
                    <div>
                        <div style="font-weight:600;color:<?= $isPresent ? 'var(--text-primary)' : 'var(--text-muted)' ?>;">
                            <?= sanitize($es['name']) ?></div>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php foreach ($students as $rank => $s): ?>
                        <tr>
                            <td>
                                <?php if ($rank === 0): ?>     <?php elseif ($rank === 1): ?>     <?php elseif ($rank === 2): ?>
                                <?php else:
                                    echo $rank + 1; endif; ?>
                            </td>
                            ...
                            <td>
                                ...
                                <span
                                    style="color:<?= $pct >= 70 ? 'var(--accent-green)' : ($pct >= 40 ? '#ffc107' : '#ff3860') ?>;font-weight:600;">
                                    <?= $pct ?>%
                                </span>
                            </td>
                            <?php foreach ($questions as $q):
                                ...
                                ?>
                                <td
                                    style="text-align:center;background:<?= $cellBg ?>;color:<?= $cellColor ?>;font-weight:700;font-size:0.8rem;">
                                    <?= $cellText ?>
                                </td>
                            <?php endforeach; ?>
                        </tr>
            ...
            <div style="padding:4px 0;">
                ...
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;">
                    <div style="display:flex;gap:12px;align-items:flex-start;flex:1;">
                        <span
                            style="background:<?= $qAccuracy >= 60 ? 'rgba(72,199,142,0.15)' : 'rgba(255,56,96,0.15)' ?>;color:<?= $qAccuracy >= 60 ? 'var(--accent-green)' : '#ff3860' ?>;padding:6px 12px;border-radius:8px;font-weight:700;font-size:0.85rem;white-space:nowrap;">
                            Q<?= $i + 1 ?>
                        </span>
                        <div>
                            <div style="font-weight:600;font-size:1rem;line-height:1.5;">
                                <?= sanitize($q['question_text']) ?></div>
                            <span style="font-size:0.75rem;color:var(--text-muted);"><?= sanitize($q['topic_name']) ?> • ⏱
                                <?= $q['time_limit_seconds'] ?>s</span>
                        </div>
                    </div>
                    <div style="text-align:right;flex-shrink:0;margin-left:16px;">
                        <div
                            style="font-size:1.3rem;font-weight:700;color:<?= $qAccuracy >= 60 ? 'var(--accent-green)' : '#ff3860' ?>;">
                            <?= $qAccuracy ?>%</div>
                        <div style="font-size:0.75rem;color:var(--text-muted);"><?= $correctCount ?>/<?= $totalStudents ?>
                            correct</div>
                    </div>
                </div>
                ...
                <div style="display:grid;gap:8px;margin-bottom:16px;">
                    <?php foreach (['a', 'b', 'c', 'd'] as $opt):
                        ...
                        ?>
                        <div
                            style="position:relative;padding:12px 16px;border-radius:8px;background:<?= $bg ?>;border:1px solid <?= $border ?>;overflow:hidden;">
                            <!-- Background bar -->
                            <div
                                style="position:absolute;top:0;left:0;bottom:0;width:<?= $optPct ?>%;background:<?= $barColor ?>;opacity:0.15;transition:width 0.6s;">
                            </div>
                            <div style="position:relative;display:flex;justify-content:space-between;align-items:center;">
                                <div style="display:flex;gap:8px;align-items:center;">
                                    <span
                                        style="font-weight:700;text-transform:uppercase;opacity:0.5;font-size:0.85rem;"><?= $opt ?>.</span>
                ...
                <details style="margin-top:8px;">
                    <summary
                        style="cursor:pointer;color:var(--accent-blue);font-size:0.85rem;font-weight:600;padding:4px 0;">
                    ...
                    <div style="margin-top:10px;display:flex;flex-wrap:wrap;gap:6px;">
                        <?php foreach ($students as $s):
                            ...
                            ?>
                            <span
                                style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:20px;background:<?= $chipBg ?>;font-size:0.75rem;"
                                title="<?= sanitize($s['enrollment_no']) ?>">
